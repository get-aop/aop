...
describe("summarizeToolArguments", () => {
  ...
  test("formats Claude Agent and Skill context", () => {
    expect(summarizeToolArguments("agent", { description: "Inspect the chat renderer" })).toBe(
      "Inspect the chat renderer",
    );
    expect(summarizeToolArguments("skill", { skill: "frontend-design" })).toBe("frontend-design");
  });
