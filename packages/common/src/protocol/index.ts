...
export const ErrorCode = {
  ...
  CHECKER_EVIDENCE_MISSING: "checker_evidence_missing",
...
const RuntimeAliasSchema = z
  .string()
  .trim()
  .regex(/^[A-Za-z0-9._/-]+$/, "Runtime alias must be a single command token")
  .optional();
...
export const StepAgentSchema = z.object({
  ...
  browserControl: z.boolean().optional(),
  computerControl: z.boolean().optional(),
  runtimeAlias: RuntimeAliasSchema,
  /** When set, spawn agent/verification on this SSH execution host. */
  execHostId: z.string().trim().min(1).optional(),
});
