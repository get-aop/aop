...
export class WorktreeOps {
  ...
  async handoff(
    ...
    const branch = await this.resolveWorktreeBranch(worktreePath, metadata);
    const commitSha = await this.resolveHandoffCommit(metadata.baseCommit, branch);
  ...
  async getCurrentBranch(taskId: string, fallbackBranch: string): Promise<string> {
    validateTaskId(taskId);
    ...
    const worktreePath = `${this.worktreesDir}/${taskId}`;
    const result = await this.executor.execRaw(["branch", "--show-current"], worktreePath);
    return result.exitCode === 0 && result.stdout ? result.stdout : fallbackBranch;
  }
  ...
  private async resolveWorktreeBranch(
    worktreePath: string,
    metadata: WorktreeMetadata,
  ): Promise<string> {
    const result = await this.executor.execRaw(["branch", "--show-current"], worktreePath);
    return result.exitCode === 0 && result.stdout ? result.stdout : metadata.branch;
  }

  private async resolveHandoffCommit(baseCommit: string, branch: string): Promise<string | null> {
    const headCommit = await this.branchOps.getCommit(branch);
    return headCommit === baseCommit ? null : headCommit;
  }
}
