...
describe("ResumeDialog - input mode", () => {
  test("renders input mode with resume trust guidance", async () => {
    ...
    expect(screen.getByText("Your response is sent to the paused Pi run.")).toBeDefined();
  });
  ...
  test("submits resume input in input mode", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("resume-confirm")).toBeDefined());

    fireEvent.change(screen.getByTestId("resume-input"), { target: { value: "Continue" } });
    fireEvent.click(screen.getByTestId("resume-confirm"));

    await waitFor(() => expect(onConfirm).toHaveBeenCalledWith("Continue"));
  });
