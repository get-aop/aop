...
describe("status/handlers", () => {
  ...
  test("reports global capacity from active workers and license worker limit", async () => {
    ...
    await db
      ...
      .values(
        Array.from({ length: 4 }, (_, index) => ({
          ...
          artifact_path: `/tmp/.aop/agents/agent-${index + 1}`,
          source_kind: "manual",
          source_ref: null,
          created_at: now,
          updated_at: now,
    ...
    expect(status.globalCapacity).toEqual({ working: 4, max: 4 });
  });
