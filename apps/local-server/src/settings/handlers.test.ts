...
describe("settings/handlers", () => {
  ...
  describe("setSetting", () => {
    ...
    test("accepts a HTTPS license server URL for hosted validation", async () => {
      const result = await setSetting(ctx, "license_server_url", "https://license.example.com");

      expect(result.success).toBe(true);
      if (!result.success) return;
      expect(result.value).toBe("https://license.example.com");
      expect(await ctx.settingsRepository.get(SettingKey.LICENSE_SERVER_URL)).toBe(
        "https://license.example.com",
      );
    });

    test("rejects invalid license server URLs", async () => {
      const result = await setSetting(ctx, "license_server_url", "not-a-url");

      expect(result.success).toBe(false);
      if (result.success) return;
      expect(result.error.code).toBe("INVALID_VALUE");
      expect(result.error.key).toBe("license_server_url");
    });

    test("rejects direct writes to license-managed settings", async () => {
      const result = await setSetting(ctx, "license_entitlement_json", '{"plan":"team"}');

      expect(result.success).toBe(false);
      if (result.success) return;
      expect(result.error.code).toBe("READONLY_KEY");
      expect(result.error.key).toBe("license_entitlement_json");
    });
