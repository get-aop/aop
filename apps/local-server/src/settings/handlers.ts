...
const validateUrlSetting = (key: SettingKey, value: string): SetSettingError | null => {
  ...
  if (key === SettingKey.LICENSE_SERVER_URL) {
    return validateHttpUrl(key, value, [
      "A valid HTTP(S) URL like https://license.example.com or http://127.0.0.1:4320",
    ]);
  }
...
const validateHttpUrl = (
  ...
  try {
    ...
    // Fall through to the shared invalid-value shape.
  }
...
export const setSetting = async (
  ...
  if (isLicenseManagedSettingKey(key)) {
    return { success: false, error: { code: "READONLY_KEY", key } };
  }
...
export const setAllSettings = async (
  ...
  for (const entry of entries) {
    ...
    if (isLicenseManagedSettingKey(entry.key)) {
      return { success: false, error: { code: "READONLY_KEY", key: entry.key } };
    }
