...
describe("task/repository", () => {
  ...
  const assignTaskToAgent = async (
    taskId: string,
    options: {
      agentId?: string;
      repoId?: string;
      agentStatus?: "active" | "archived";
      includeRepoMembership?: boolean;
    } = {},
  ): Promise<string> => {
    const agentId = options.agentId ?? `agent-${taskId}`;
    ...
    if (options.includeRepoMembership ?? true) {
      await db
        ...
        .onConflict((oc) => oc.columns(["agent_id", "repo_id"]).doNothing())
        .execute();
    }
  ...
  const captureTaskEvents = () => {
    ...
    return { events, repoWithEvents: createTaskRepository(db, { eventEmitter }) };
  };
  ...
  describe("events", () => {
    test("status-changed events include the current assignment board column", async () => {
      const { events, repoWithEvents } = captureTaskEvents();
      await createReadyTask("task-1", "repo-1", "changes/feat-1", new Date().toISOString());
      await assignTaskToAgent("task-1", { agentId: "agent-1" });
      await db
      ...
      await repo.update("task-1", { status: "WORKING" });
      await repoWithEvents.refresh();
      ...
      if (event?.type === "task-status-changed") {
        expect(event.task.boardColumn).toBe("IN_PROGRESS");
        expect(event.task.assignedAgentId).toBe("agent-1");
      }
    });
  });
  ...
  describe("getByChangePath", () => {
    test("returns task by repo_id and change_path", async () => {
      ...
      const task = await repo.getByChangePath("repo-1", "docs/tasks/feat-1");
  ...
  describe("getNextExecutable", () => {
    ...
    test("skips READY tasks assigned to inactive agents", async () => {
      const now = new Date();
      await createReadyTask("task-assigned", "repo-1", "changes/assigned", now.toISOString());
      await assignTaskToAgent("task-assigned", { agentStatus: "archived" });
      await createReadyTask(
        "task-legacy",
        "repo-1",
        "changes/legacy",
      ...
      const task = await repo.getNextExecutable({
        globalMax: 5,
        getRepoMax: async () => 2,
      });

      expect(task?.id).toBe("task-legacy");
    });

    test("skips READY tasks when the assigned agent is already running another task", async () => {
      const now = new Date();
      await createReadyTask("task-ready", "repo-1", "changes/ready", now.toISOString());
      const agentId = await assignTaskToAgent("task-ready");
      await createTestTask(db, "task-working", "repo-1", "changes/working", "WORKING");
      await assignTaskToAgent("task-working", { agentId });
      await createReadyTask(
        "task-fallback",
        "repo-1",
        "changes/fallback",
    ...
    test("returns Jira blocker refs for terminal Jira dependencies", async () => {
      const externalIssueStore = createExternalIssueStore(db);
      await createTestTask(db, "task-upstream", "repo-1", "changes/upstream", "BLOCKED");
      await createReadyTask("task-blocked", "repo-1", "changes/blocked", new Date().toISOString());
      await externalIssueStore.replaceTaskDependencies({
        taskId: "task-blocked",
        source: "jira_blocks",
      ...
      await externalIssueStore.upsertTaskSource({
        ...
        repoId: "repo-1",
        provider: "jira",
        externalId: "jira-get-49",
        externalRef: "GET-49",
        externalUrl: "https://acme.atlassian.net/browse/GET-49",
        titleSnapshot: "Upstream Jira task",
      });
      ...
      expect(dependencyState).toEqual({
        dependencyState: "blocked",
        blockedByTaskIds: ["task-upstream"],
        blockedByRefs: ["GET-49"],
      });
    });

    test("deduplicates blocker task ids for dependencies mirrored by multiple providers", async () => {
      const externalIssueStore = createExternalIssueStore(db);
      await createTestTask(db, "task-upstream", "repo-1", "changes/upstream", "BLOCKED");
      await createReadyTask("task-blocked", "repo-1", "changes/blocked", new Date().toISOString());
      await externalIssueStore.replaceTaskDependencies({
      ...
      await externalIssueStore.upsertTaskSource({
        ...
        repoId: "repo-1",
        provider: "linear",
        externalId: "linear-get-49",
        externalRef: "LIN-49",
        externalUrl: "https://linear.app/acme/issue/LIN-49/upstream",
        titleSnapshot: "Upstream Linear task",
      });
      await externalIssueStore.upsertTaskSource({
        taskId: "task-upstream",
        repoId: "repo-1",
      ...
      expect(dependencyState.dependencyState).toBe("blocked");
      expect(dependencyState.blockedByTaskIds).toEqual(["task-upstream"]);
      expect(dependencyState.blockedByRefs.sort()).toEqual(["GET-49", "LIN-49"]);
    });
    ...
    test("keeps dependent tasks waiting after repository recreation while upstream runtime is still working", async () => {
      ...
      await updateTaskDocStatus(join(repoPath, "docs/tasks/upstream/task.md"), "DONE");
