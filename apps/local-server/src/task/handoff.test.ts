...
describe("handoffCompletedTask", () => {
  ...
  test("hands off code changes without touching repo-local task docs", async () => {
    const db = await createTestDb();
    ...
    const worktree = await gitManager.createWorktree("task-2", "main", "benchmark-filter-by-tag");
    ...
    expect(
      (await Bun.$`git show benchmark-filter-by-tag:src/notes.ts`.cwd(repo.path).text()).trim(),
