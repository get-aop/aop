...
describe("sse-stream", () => {
  ...
  test("serializes concurrent writes with monotonically increasing event ids", async () => {
    let activeWrites = 0;
    ...
    const sse = createSSEStreamHelper(stream);
    const first = sse.sendEvent("connected", { ok: true });
    const second = sse.sendEvent("assistant-progress", { n: 1 });
    const third = sse.sendEvent("assistant-progress", { n: 2 });

    // First write is held open; later callers must queue rather than start.
    await Bun.sleep(5);
    ...
    expect(maxActiveWrites).toBe(1);
    expect(written.map((event) => event.id)).toEqual(["0", "1", "2"]);
    expect(written.map((event) => event.event)).toEqual([
      "connected",
      "assistant-progress",
      "assistant-progress",
    ]);
