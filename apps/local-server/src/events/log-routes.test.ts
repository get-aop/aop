...
describe("log-routes", () => {
  ...
  describe("GET /api/executions/:executionId/logs", () => {
    ...
    it("streams buffered Grok tokens as one coherent line", async () => {
      await createExecWithStep("exec-grok-buffer", "step-grok-buffer");
      const responsePromise = app.request("/api/executions/exec-grok-buffer/logs");

      await new Promise((resolve) => setTimeout(resolve, 50));
      ctx.logBuffer.push("step-grok-buffer", JSON.stringify({ type: "text", data: "Extract" }));
      ctx.logBuffer.push(
        "step-grok-buffer",
        JSON.stringify({ type: "text", data: "ing failure details." }),
      );
      ctx.logBuffer.markComplete("step-grok-buffer", "completed");

      const events = parseSSEEvents(await (await responsePromise).text());
      const contents = events
        .map((event) => JSON.parse(event.data))
        .filter((event) => event.type === "log")
        .map((event) => event.content);

      expect(contents).toEqual(["Extracting failure details."]);
    });

    it("replays persisted logs for running execution without a current log file", async () => {
      await createExecWithStep("exec-1", "step-1");
      await ctx.executionRepository.saveStepLogs([
        {
          step_execution_id: "step-1",
          content: makeRawJsonl("persisted running line"),
          created_at: "2024-01-01T00:00:01Z",
        },
      ]);

      const logsDir = join(tmpdir(), `aop-log-routes-${Date.now()}`);
      mkdirSync(logsDir, { recursive: true });
      const appWithEmptyLogDir = new Hono();
      appWithEmptyLogDir.get(
        "/api/executions/:executionId/logs",
        createLogStreamHandler(ctx, { logsDir }),
      );

      const responsePromise = appWithEmptyLogDir.request("/api/executions/exec-1/logs");
      ...
      const replayEvent = JSON.parse(events[0]?.data ?? "{}");
      expect(replayEvent.type).toBe("replay");
      expect(replayEvent.lines[0].content).toBe("persisted running line");
    ...
    it("sends complete event with cancelled status", async () => {
      ...
      const replayEvent = JSON.parse(events[0]?.data ?? "{}");
      expect(replayEvent.type).toBe("replay");
      expect(replayEvent.lines).toEqual([]);
    ...
    it("replays buffered latest-step logs for completed execution before persistence catches up", async () => {
      await createTestRepo(db, "repo-1", "/test/repo");
      await createTestTask(db, "task-1", "repo-1", "changes/feat-1", "DONE");
      await ctx.executionRepository.createExecution({
        id: "exec-1",
      ...
      ctx.logBuffer.push("step-1", makeRawJsonl("buffered line 1"));
      ctx.logBuffer.push("step-1", makeRawJsonl("buffered line 2"));
      ctx.logBuffer.markComplete("step-1", "completed");
      ...
      expect(replayEvent.lines[0].content).toBe("buffered line 1");
      expect(replayEvent.lines[1].content).toBe("buffered line 2");
    ...
    it("replays rewritten latest-step log files for completed interactive resumes", async () => {
      const logsDir = join(tmpdir(), `aop-log-routes-rotated-${Date.now()}`);
      mkdirSync(logsDir, { recursive: true });
      const appWithLogDir = new Hono();
      appWithLogDir.get(
        "/api/executions/:executionId/logs",
        createLogStreamHandler(ctx, { logsDir }),
      );
      ...
      await ctx.executionRepository.saveStepLogs([
        ...
        {
          step_execution_id: "step-1",
          content: makeRawJsonl("Which sidebar should change?"),
          created_at: "2024-01-01T00:00:01Z",
        },
        {
          step_execution_id: "step-1",
          content: JSON.stringify({ type: "step_finish", part: { reason: "stop" } }),
          created_at: "2024-01-01T00:00:02Z",
        },
      ]);
      writeFileSync(
        join(logsDir, "step-1.jsonl"),
        [
      ...
      const res = await appWithLogDir.request("/api/executions/exec-1/logs");
      const text = await res.text();
      const events = parseSSEEvents(text);
      const replayEvent = JSON.parse(events[0]?.data ?? "{}");

      expect(replayEvent.lines.map((line: { content: string }) => line.content)).toContain(
        "Which sidebar should change?",
      );
      expect(replayEvent.lines.map((line: { content: string }) => line.content)).toContain(
        "# Plan",
      );
    ...
    it("tags replay lines with stepExecutionId across multiple steps", async () => {
      await createTestRepo(db, "repo-1", "/test/repo");
      await createTestTask(db, "task-1", "repo-1", "changes/feat-1", "DONE");
      await ctx.executionRepository.createExecution({
        id: "exec-1",
      ...
      await ctx.executionRepository.saveStepLogs([
        {
          step_execution_id: "step-1",
          content: makeRawJsonl("iterate output"),
          created_at: "2024-01-01T00:00:01Z",
        },
        {
          step_execution_id: "step-2",
          content: makeRawJsonl("run tests output"),
          created_at: "2024-01-01T00:00:02Z",
        },
      ]);
      ...
      expect(replayEvent.type).toBe("replay");
      expect(replayEvent.lines).toHaveLength(2);
      expect(replayEvent.lines[0].content).toBe("iterate output");
      expect(replayEvent.lines[0].stepExecutionId).toBe("step-1");
      expect(replayEvent.lines[1].content).toBe("run tests output");
      expect(replayEvent.lines[1].stepExecutionId).toBe("step-2");
    });
...
describe("log-routes file-based streaming", () => {
  ...
  it("replays Grok streaming tokens as coherent log lines", async () => {
    const stepId = "step-grok";
    await createRunningExecution("exec-grok", stepId, 99994);

    writeJsonl(`${stepId}.jsonl`, [
      { type: "thought", data: "private reasoning" },
      { type: "text", data: "Extract" },
      { type: "text", data: "ing" },
      { type: "text", data: " failure" },
    ...
    const res = await app.request("/api/executions/exec-grok/logs");
    const events = parseSSEEvents(await res.text());
    const replayEvent = events
      .map((event) => JSON.parse(event.data))
      .find((event) => event.type === "replay");

    expect(replayEvent.lines.map((line: { content: string }) => line.content)).toEqual([
      "Extracting failure details.",
    ]);
  });

  it("sends an initial replay event when a live log file has no visible lines", async () => {
    const stepId = "step-empty";
    await createRunningExecution("exec-empty", stepId, 99995);
    writeFileSync(join(testDir, `${stepId}.jsonl`), "");

    setupFileRoute(() => false);

    const res = await app.request("/api/executions/exec-empty/logs");
    expect(res.status).toBe(200);
    ...
    expect(parsed[0].type).toBe("replay");
  ...
  it("keeps a Grok text run coherent across file polls", async () => {
    const stepId = "step-grok-live";
    await createRunningExecution("exec-grok-live", stepId, 99993);
    writeJsonl(`${stepId}.jsonl`, [{ type: "thought", data: "private reasoning" }]);

    let alive = true;
    setupFileRoute(() => alive);
    const responsePromise = app.request("/api/executions/exec-grok-live/logs");
    const logFile = join(testDir, `${stepId}.jsonl`);

    await new Promise((resolve) => setTimeout(resolve, 75));
    writeFileSync(logFile, `${JSON.stringify({ type: "text", data: "Extract" })}\n`, {
      flag: "a",
    });
    await new Promise((resolve) => setTimeout(resolve, 75));
    writeFileSync(logFile, `${JSON.stringify({ type: "text", data: "ing failure details." })}\n`, {
      flag: "a",
    });
    await new Promise((resolve) => setTimeout(resolve, 75));
    writeFileSync(logFile, `${JSON.stringify({ type: "end", stopReason: "EndTurn" })}\n`, {
      flag: "a",
    });

    await ctx.executionRepository.updateExecution("exec-grok-live", {
      status: ExecutionStatus.COMPLETED,
      completed_at: new Date().toISOString(),
    });
    alive = false;
