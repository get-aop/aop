...
import type { Database, NewWorkflow, Workflow, WorkflowSource } from "../db/schema.ts";
...
export interface WorkflowRepository {
  ...
  listActiveBuiltinNames: () => Promise<string[]>;
...
const normalizeWorkflowSource = (source: unknown): WorkflowSource =>
  source === "builtin" ? "builtin" : "user";

const normalizeWorkflow = (workflow: Workflow): Workflow => ({
  ...workflow,
  source: normalizeWorkflowSource(workflow.source),
...
export const createWorkflowRepository = (db: Kysely<Database>): WorkflowRepository => {
  ...
  return {
    ...
    listActive: async (): Promise<Workflow[]> => {
      const rows = await db
        .selectFrom("workflows")
        .selectAll()
        .where("active", "=", true)
    ...
    listActiveBuiltinNames: async (): Promise<string[]> => {
      const rows = await db
        .selectFrom("workflows")
        .select("name")
        .where("active", "=", true)
        ...
        return normalizeWorkflow(
          await db
            ...
            .set({
              ...
              source: workflow.source ?? existing.source,
