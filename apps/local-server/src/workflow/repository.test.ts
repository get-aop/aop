...
describe("WorkflowRepository", () => {
  ...
  test("upsert inserts new workflows and updates existing ones", async () => {
    ...
    expect(inserted.source).toBe("builtin");
  ...
  test("lists stale active built-in workflow names separately from user workflows", async () => {
    await repository.create({
      id: "builtin-1",
      name: "old-builtin",
      definition: "{}",
    ...
    expect(await repository.listActiveBuiltinNames()).toEqual(["old-builtin"]);
  });
