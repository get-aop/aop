...
beforeAll(async () => {
  [
    ...
    loadOfficialWorkflow("aop-default-gpt"),
...
describe("WorkflowStateMachine", () => {
  ...
  describe("aop-default-gpt full flow simulation", () => {
    test("implement failure blocks instead of looping via __none__", () => {
    ...
    test("blocks with a clear reason when tests finish without a required signal", () => {
      const sm = createWorkflowStateMachine(aopDefault);

      const result = sm.evaluateTransition("run-tests", { status: "success" });
    ...
    test("plain implementation success still advances when signals are optional", () => {
      const sm = createWorkflowStateMachine(aopDefault);

      const result = asStepResult(sm.evaluateTransition("implement", { status: "success" }));

      expect(result.stepId).toBe("run-tests");
    });

    test("happy path: implement → tests → simplification → nuclear_review(PASSED) → done", () => {
      const sm = createWorkflowStateMachine(aopDefault);
      ...
      expect(trace[1]?.nextStepId).toBe("simplification");
      expect(trace[2]?.nextStepId).toBe("nuclear_review");
    ...
    test("failing tests route through debug-failures before verification can continue", () => {
      ...
      expect(trace[3]?.nextStepId).toBe("simplification");
    });

    test("repeated test/debug cycles block instead of looping forever", () => {
      const sm = createWorkflowStateMachine(aopDefault);

      const trace = simulateExecutionServiceFlow(sm, [
        { status: "success", signal: "ALL_TASKS_DONE" }, // implement → run-tests
        { status: "success", signal: "TESTS_FAIL" }, // run-tests → debug-failures
        { status: "success", signal: "FIX_COMPLETE" }, // debug-failures → run-tests, iteration 1
        { status: "success", signal: "TESTS_FAIL" }, // run-tests → debug-failures
        { status: "success", signal: "FIX_COMPLETE" }, // debug-failures → run-tests, iteration 2
        { status: "success", signal: "TESTS_FAIL" }, // run-tests → debug-failures
        { status: "success", signal: "FIX_COMPLETE" }, // debug-failures → run-tests, iteration 3
        { status: "success", signal: "TESTS_FAIL" }, // max test/debug cycles exceeded
      ]);

      expect(trace.at(-1)).toMatchObject({
        resolvedCurrentStepId: "run-tests",
        signal: "TESTS_FAIL",
        resultType: "blocked",
        iteration: 5,
      });
    ...
    test("one review cycle: nuclear_review(FAILED) → fix → quick-review(PASSED) → done", () => {
      const sm = createWorkflowStateMachine(aopDefault);
    ...
    test("two review cycles stay in fix and quick-review until the fixes pass", () => {
      ...
      const trace = simulateExecutionServiceFlow(sm, [
        { status: "success", signal: "ALL_TASKS_DONE" }, // implement → run-tests
        { status: "success", signal: "TESTS_PASS" }, // run-tests → simplification
        { status: "success", signal: "CLEANUP_COMPLETE" }, // simplification → nuclear_review
        { status: "success", signal: "REVIEW_FAILED" }, // nuclear_review → fix-issues
      ...
      expect(trace[0]?.resolvedCurrentStepId).toBe("implement");
      expect(trace[0]?.nextStepId).toBe("run-tests");
      ...
      expect(trace[2]?.resolvedCurrentStepId).toBe("simplification");
      expect(trace[2]?.nextStepId).toBe("nuclear_review");
      expect(trace[2]?.iteration).toBe(0);

      expect(trace[3]?.resolvedCurrentStepId).toBe("nuclear_review");
      expect(trace[3]?.nextStepId).toBe("fix-issues");
      ...
      expect(trace[7]?.iteration).toBe(2);
    });

    test("quick-review failures return to fix-issues", () => {
      const sm = createWorkflowStateMachine(aopDefault);
