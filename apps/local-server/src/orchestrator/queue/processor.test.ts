...
describe("QueueProcessor", () => {
  ...
  const createResolvedStep = (): { step: StepCommand; execution: ExecutionInfo } => ({
    ...
    execution: { id: "exec-1", workflowId: "aop-default-gpt" },
  });

  const createWorkflowManagementMocks = () => ({
    listWorkflowDetails: mock(async () => []),
    listStepLibrary: mock(async () => []),
    createSkillBlock: mock(async (input: CreateSkillBlockInput) => ({
      ...input,
      source: "user" as const,
    })),
    createWorkflowFromSteps: mock(async () => ({
      id: "workflow-custom",
      name: "custom",
      version: 1,
      active: true,
    ...
    deleteSkillBlock: mock(async () => undefined),
    deleteWorkflow: mock(async () => undefined),
  });

  const assignTaskToActiveAgent = async (taskId: string, agentId = `agent-${taskId}`) => {
  ...
  test("starts the next READY task with the local workflow service", async () => {
    ...
    const workflowService = {
      listWorkflows: mock(async () => ["aop-default-gpt", "simple"]),
      ...createWorkflowManagementMocks(),
      startTask: mock(
  ...
  test("reverts task to READY when workflow start does not return a runnable step", async () => {
    ...
    const workflowService = {
      listWorkflows: mock(async () => ["aop-default-gpt"]),
      ...createWorkflowManagementMocks(),
      startTask: mock(
        async (): Promise<TaskReadyResponse> => ({
          status: "READY",
  ...
  test("resumes an assigned RESUMING task without counting itself as active agent work", async () => {
    await createTestRepo(db, "repo-1", "/test/repo");
    await createTestTask(db, "task-1", "repo-1", "changes/feat-1", "RESUMING");
    await ctx.taskRepository.update("task-1", { resume_input: "Continue" });
    await assignTaskToActiveAgent("task-1", "agent-1");
    await ctx.executionRepository.createExecution({
      id: "exec-1",
      task_id: "task-1",
      workflow_id: "release-bump",
      status: "running",
      visited_steps: JSON.stringify(["release_bump"]),
      iteration: 0,
      started_at: new Date().toISOString(),
    });
    await ctx.executionRepository.createStepExecution({
    ...
    const resolved = createResolvedStep();
    const workflowService = {
      listWorkflows: mock(async () => ["release-bump"]),
      ...createWorkflowManagementMocks(),
      startTask: mock(async (): Promise<TaskReadyResponse> => ({ status: "DONE" })),
      completeStep: mock(
        async (): Promise<StepCompleteResponse> => ({ taskStatus: "DONE", step: null }),
  ...
  test("clamps configured global concurrency to the five-developer execution cap", async () => {
    await createTestRepo(db, "repo-1", "/test/repo", { maxConcurrentTasks: 10 });
    await ctx.settingsRepository.set(SettingKey.MAX_CONCURRENT_TASKS, "9");

    for (let index = 1; index <= 5; index += 1) {
      await createTestTask(db, `working-${index}`, "repo-1", `changes/working-${index}`, "READY");
      await ctx.taskRepository.update(`working-${index}`, {
        status: "WORKING",
        worktree_path: `/tmp/worktree-${index}`,
      });
    }

    await createTestTask(db, "task-ready", "repo-1", "changes/task-ready", "READY");
    await ctx.taskRepository.update("task-ready", {
      status: "READY",
    ...
    expect(result).toBeNull();
    expect(workflowService.startTask).not.toHaveBeenCalled();
  });
});
