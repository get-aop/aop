...
describe("createTemplateContext", () => {
  test("creates context with all fields", () => {
    ...
    expect(context.task.docsDir).toBe("changes/my-change");
    expect(context.step.type).toBe("implement");
    expect(context.step.executionId).toBe("exec-456");
    expect(context.step.iteration).toBe(2);
    expect(context.task.attachments).toEqual([]);
  });

  test("passes task attachments through", () => {
    const attachments = [{ label: "#image1", path: "/docs/tasks/x/attachments/task-image-1.png" }];
    const context = createTemplateContext({
      worktreePath: "/path/to/worktree",
      worktreeBranch: "feature-branch",
      taskId: "task-123",
    ...
    expect(context.task.attachments).toEqual(attachments);
  });
});
...
describe("resolveTemplate", () => {
  ...
  test("resolves repository context placeholders", () => {
    const context = createTemplateContext({
      worktreePath: "/repo/.worktrees/task-1",
      worktreeBranch: "aop/task-1",
      taskId: "task-1",
    ...
    const template =
      "{{#each task.repositories}}{{this.repoId}}:{{this.assignment}}:{{this.path}}:{{this.writable}}\n{{/each}}";
    const result = resolveTemplate(template, context);

    expect(result).toContain("aop-mono:primary:/repo:true");
    expect(result).toContain("shared-ui:supporting:/deps/shared-ui:false");
  });

  test("resolves task attachment placeholders with each block", () => {
    const context = createTemplateContext({
      worktreePath: "/repo/.worktrees/task-1",
      worktreeBranch: "aop/task-1",
      taskId: "task-1",
      changePath: "changes/feature",
      attachments: [
        { label: "#image1", path: "/docs/tasks/x/attachments/task-image-1.png" },
        { label: "#image2", path: "/docs/tasks/x/attachments/task-image-2.png" },
      ],
      stepType: "implement",
      executionId: "exec-1",
      iteration: 0,
    });
    const template = "{{#each task.attachments}}{{this.label}}: {{this.path}}\n{{/each}}";
    const result = resolveTemplate(template, context);

    expect(result).toContain("#image1: /docs/tasks/x/attachments/task-image-1.png");
    expect(result).toContain("#image2: /docs/tasks/x/attachments/task-image-2.png");
  });

  test("omits attachment sections when the task has none", () => {
    const template = "{{#if task.attachments.length}}## Attached Images{{/if}}";
    const result = resolveTemplate(template, baseContext);
...
describe("end-to-end template resolution", () => {
  ...
  test("resolves run-tests template with signals to exact expected output", async () => {
    ...
    expect(result).toContain("marker/text-only diffs");
    expect(result).toContain("npx --yes bun");
  ...
  test("resolves implement template with image attachments listed", async () => {
    const template = await loader.load("implement.md.hbs");
    const context = createTemplateContext({
      worktreePath: "/repo/.worktrees/task-9",
      worktreeBranch: "aop/task-9",
      taskId: "task-9",
      changePath: "docs/tasks/icon-fix",
      docsDir: "/home/user/.aop/repos/repo-1/tasks/icon-fix",
      attachments: [
        {
          label: "#image1",
          path: "/home/user/.aop/repos/repo-1/tasks/icon-fix/attachments/task-image-1.png",
        },
      ],
      stepType: "implement",
      executionId: "exec-9",
    ...
    expect(result).toContain("View each image file before working");
    expect(result).toContain(
      "- #image1: `/home/user/.aop/repos/repo-1/tasks/icon-fix/attachments/task-image-1.png`",
    );
  });

  test("resolves implement template without attachments omits the images section", async () => {
    const template = await loader.load("implement.md.hbs");
    const context = createTemplateContext({
      worktreePath: "/repo/.worktrees/task-9",
      worktreeBranch: "aop/task-9",
