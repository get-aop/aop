...
describe("integrations/linear/handlers", () => {
  ...
  test("refreshes expired OAuth tokens before testing the connection", async () => {
    const expiredTokens = {
    ...
    const save = mock(async () => {});
    const refreshTokens = mock(async () => freshTokens);
    const testConnectionWithToken = mock(async () => ({
      ok: true,
      organizationName: "Acme",
      userName: "Jane Doe",
      userEmail: "jane@example.com",
    ...
    const handlers = createLinearHandlers({
      ...
      exchangeCodeForTokens: async () => freshTokens,
      refreshTokens,
    ...
    expect(save).toHaveBeenCalledWith(freshTokens);
    expect(testConnectionWithToken).toHaveBeenCalledWith("fresh-access-token");
  });
});
