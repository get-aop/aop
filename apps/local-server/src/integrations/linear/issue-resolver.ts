...
const normalizeIssue = (issue: LinearRawIssue): LinearResolvedIssue => ({
  ...
  team:
    issue.team?.key && issue.team.name
      ...
      : null,
  url: issue.url,
  blocks: (issue.inverseRelations?.nodes ?? []).reduce<LinearIssueSummary[]>((blocks, relation) => {
    if (normalizeRelationType(relation.type) !== "blocks" || !relation.issue) {
      return blocks;
    }

    if (isTerminalIssueState(relation.issue.state?.type)) {
      return blocks;
    }

    blocks.push(toIssueSummary(relation.issue));
    return blocks;
  }, []),
});
...
const isTerminalIssueState = (type: string | null | undefined): boolean => {
