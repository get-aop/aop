...
import { JiraOAuthHandlersError } from "./oauth-handlers.ts";
import type { JiraOAuthRoutesDeps } from "./oauth-types.ts";

const JIRA_OAUTH_CHANNEL = "aop-jira-oauth";

export const createJiraOAuthRoutes = (deps: JiraOAuthRoutesDeps) => {
  const app = new Hono();
...
const handleOAuthCallback = async (c: Context, deps: JiraOAuthRoutesDeps) => {
  try {
...
const buildOAuthCallbackPage = (params: { connected: boolean; error?: string }): string => {
  const title = params.connected ? "Jira connected" : "Jira connection failed";
  const message = params.connected
    ? "You can close this tab. AOP will refresh the Settings page automatically."
    : (params.error ?? "Jira connection failed");
  const serializedPayload = JSON.stringify(
    params.connected
      ? { type: "jira-oauth-complete" }
      : { type: "jira-oauth-error", error: message },
  );
...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      try {
        const channel = new BroadcastChannel("${JIRA_OAUTH_CHANNEL}");
...
const toErrorResponse = (c: Context, error: unknown) => {
  if (error instanceof JiraOAuthHandlersError) {
