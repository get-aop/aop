...
describe("createWorktree", () => {
  ...
  test("creates a worktree from the task base_branch when set", async () => {
    ...
    const ctx = {
      repoId: "repo-1",
      repoPath: "/tmp/repo-1",
      changePath: aopPaths.repoTask("repo-1", "restore-auth-flow"),
      task: {
        id: "task-1",
    ...
    expect(createWorktreeSpy).toHaveBeenCalledWith("task-1", "release", "restore-auth-flow");
  });

  test("creates a worktree from the repo default branch when base_branch is null", async () => {
    const worktree: WorktreeInfo = {
      path: "/tmp/worktree/task-1b",
      branch: "delivery-feature",
      baseBranch: "main",
      baseCommit: "def456",
    };
    const initSpy = spyOn(GitManager.prototype, "init").mockResolvedValue(undefined);
    ...
    const ctx = {
      repoId: "repo-1b",
      repoPath: "/tmp/repo-1b",
      changePath: aopPaths.repoTask("repo-1b", "delivery-feature"),
      task: {
        id: "task-1b",
        change_path: "docs/tasks/delivery-feature",
        base_branch: null,
      },
    } as ExecutorContext;
    ...
    expect(createWorktreeSpy).toHaveBeenCalledWith("task-1b", "main", "delivery-feature");
  });
  ...
  test("returns the existing worktree path when the worktree already exists", async () => {
    ...
    const getWorktreeBranchSpy = spyOn(GitManager.prototype, "getWorktreeBranch").mockResolvedValue(
      "renamed-dashboard",
    );
    ...
    const ctx = {
      repoId: "repo-2",
      repoPath: "/tmp/repo-2",
      changePath: aopPaths.repoTask("repo-2", "delivery-dashboard"),
      task: {
        id: "task-2",
        change_path: "docs/tasks/delivery-dashboard",
        base_branch: null,
      },
    } as ExecutorContext;
    ...
    await expect(createWorktree(ctx)).resolves.toEqual({
      ...
      branch: "renamed-dashboard",
      baseBranch: "main",
      baseCommit: "",
    });
    expect(initSpy).toHaveBeenCalledTimes(1);
    expect(getDefaultBranchSpy).toHaveBeenCalledTimes(1);
    expect(createWorktreeSpy).toHaveBeenCalledWith("task-2", "main", "delivery-dashboard");
    expect(getWorktreeBranchSpy).toHaveBeenCalledWith("task-2", "delivery-dashboard");
  });
  ...
  test("rethrows unexpected worktree creation errors", async () => {
    ...
    const ctx = {
      repoId: "repo-3",
      repoPath: "/tmp/repo-3",
      changePath: aopPaths.repoTask("repo-3", "delivery-dashboard"),
      task: {
        id: "task-3",
        change_path: "docs/tasks/delivery-dashboard",
        base_branch: null,
    ...
    expect(initSpy).toHaveBeenCalledTimes(1);
    expect(getDefaultBranchSpy).toHaveBeenCalledTimes(1);
    expect(createWorktreeSpy).toHaveBeenCalledWith("task-3", "main", "delivery-dashboard");
  });
});
