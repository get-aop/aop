...
describe("completion-handler", () => {
  ...
  test("processAgentCompletion preserves provider-supplied usage", async () => {
  ...
  test("finalizeExecutionAndGetNextStep marks DONE tasks pending handoff approval when enabled", async () => {
    await createTestRepo(db, "repo-1", join(tempDir, "repo-1"));
    await createTestTask(db, "task-1", "repo-1", "changes/task-1", "WORKING");
    await ctx.settingsRepository.set(SettingKey.HANDOFF_REQUIRES_APPROVAL, "true");
    ctx.workflowService = {
      ...ctx.workflowService,
    ...
    expect(task?.handoff_pending_approval).toBe(true);
  });

  test("finalizeExecutionAndGetNextStep lets task-level handoff approval override global settings", async () => {
    await createTestRepo(db, "repo-1", join(tempDir, "repo-1"));
    await createTestTask(db, "task-1", "repo-1", "changes/task-1", "WORKING");
    await ctx.settingsRepository.set(SettingKey.HANDOFF_REQUIRES_APPROVAL, "true");
    await ctx.taskRepository.update("task-1", {
      handoff_requires_approval_override: false,
      worktree_path: null,
    });
    ctx.workflowService = {
      ...ctx.workflowService,
