...
const applyRecoveryAction = async (
  ...
  // Remote v1: best-effort pkill on configured hosts before marking the step done/failed.
  await cleanupRemoteStepOnConfiguredHosts(ctx, step.id);
  ...
  await ctx.runtimeEventRepository.insertMany([
    {
      ...
      title: "Runtime session interrupted",
      message: "AOP could not recover the running worker process or its log file.",
      tool_name: null,
      status: "interrupted",
      ...
      source_index: 0,
      occurred_at: now,
      metadata_json: JSON.stringify({ reason: "missing_process_and_log" }),
    },
  ]);
...
const persistStepLogsFromOffset = async (
  ...
  try {
    ...
    await projectRuntimeEventsForStep(ctx, stepExecutionId);
