...
describe("recoverStaleTasks", () => {
  ...
  test("resets task to READY when agent is dead and no log file exists", async () => {
    ...
    const runtimeEvents = await ctx.runtimeEventRepository.listByExecutionId("exec-1");
    expect(runtimeEvents[0]?.kind).toBe("session_interrupted");
  });
  ...
  test("completes a recovered successful step and launches the next step when workflow continues", async () => {
    ...
    const nextExecution: ExecutionInfo = { id: "exec-1", workflowId: "aop-default-gpt" };
