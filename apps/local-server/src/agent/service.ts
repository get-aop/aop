...
import { isWorkerLimitReached } from "@aop/license";
...
import { assertWithinWorkerLimit } from "../license/service.ts";
import {
  AgentNameSchema,
...
export type AgentServiceError =
  | { code: "ACTIVE_AGENT_LIMIT_REACHED"; limit: number }
  | { code: "LICENSE_NOT_VALID"; message: string }
...
const checkWorkerLimit = async (
  ctx: LocalServerContext,
  agentRepository: ReturnType<typeof createAgentRepository>,
): Promise<AgentServiceResult<never> | null> => {
  const activeCount = await agentRepository.countActive();
  const limitCheck = await assertWithinWorkerLimit(ctx.licenseStorage, activeCount);
  if (!limitCheck.allowed) {
    if ("message" in limitCheck) {
      return {
        success: false,
        error: { code: "LICENSE_NOT_VALID", message: limitCheck.message },
      };
    }
    return {
      success: false,
      error: { code: "ACTIVE_AGENT_LIMIT_REACHED", limit: limitCheck.limit },
    };
  }
  return null;
};

const assertActiveWorkerSlotAvailable = (activeCount: number, maxWorkers: number | null): void => {
  if (isWorkerLimitReached(activeCount, maxWorkers)) {
    throw new ActiveWorkerLimitError({
      success: false,
      error: { code: "ACTIVE_AGENT_LIMIT_REACHED", limit: maxWorkers ?? 0 },
    });
  }
};
...
export const createAgentService = (ctx: LocalServerContext): AgentService => {
  ...
  return {
    createAgent: async (input) => {
      ...
      const limitError = await checkWorkerLimit(ctx, agentRepository);
      if (limitError) {
        return limitError;
      }
    ...
    updateAgent: async (agentId, input) => {
      ...
      if (reactivating) {
        const maxWorkers = await ctx.licenseService.getMaxActiveWorkers();
        const limitError = await checkWorkerLimit(ctx, agentRepository);
        if (limitError) {
          return limitError;
        }
        ...
        try {
          return await ctx.db.transaction().execute(async (trx) => {
            ...
            assertActiveWorkerSlotAvailable(await trxAgentRepository.countActive(), maxWorkers);
