...
describe("install.sh preflight", () => {
  ...
  test("persists hosted licensing environment variables into installed services", async () => {
    const script = await readFile(INSTALLER, "utf8");

    expect(script).toContain("AOP_LICENSE_SERVER_URL");
    expect(script).toContain("AOP_CHECKOUT_PRO_URL");
    expect(script).toContain("AOP_CHECKOUT_TEAM_URL");
  });
