...
const taskBranchName = (task: TaskInfo): string => task.change_path.split("/").at(-1) ?? task.id;

const expectBranchFileText = async (
  repoPath: string,
  branch: string,
  filePath: string,
): Promise<string> => Bun.$`git show ${`${branch}:${filePath}`}`.cwd(repoPath).text();
...
e2eDescribe("real dependency-aware concurrency", () => {
  let ctx: TestContext;
  ...
  test(
    ...
    async () => {
      ...
      await expect(Bun.file(join(repo.path, "task1.txt")).exists()).resolves.toBe(false);
      await expect(Bun.file(join(repo.path, "task2.txt")).exists()).resolves.toBe(false);
      await expect(Bun.file(join(repo.path, "task3.txt")).exists()).resolves.toBe(false);
      expect(await expectBranchFileText(repo.path, taskBranchName(firstTask), "task1.txt")).toBe(
        "Task 1 completed\n",
      );
      expect(await expectBranchFileText(repo.path, taskBranchName(secondTask), "task2.txt")).toBe(
        "Task 2 completed\n",
      );
      expect(
        await expectBranchFileText(repo.path, taskBranchName(dependentTask), "task3.txt"),
      ).toBe("Task 3 completed\n");
    },
    E2E_TIMEOUT,
  );
});
