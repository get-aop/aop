...
e2eDescribe("automatic task handoff", () => {
  let ctx: TestContext;
  ...
  test(
    "hands off DONE task changes onto the task branch without changing main",
    async () => {
      ...
      try {
        ...
        await createAndAssignE2EWorker(ctx.localServerUrl, taskBefore.repoId, taskId);
        ...
        expect(existsSync(helloInMainPath)).toBe(false);
